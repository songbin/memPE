...
static void settings_init(void) {
    ...
    settings.usePrs = false;
    memset( settings.szDBName, 0x0, DB_NAME_MAX_LEN );
    settings.db = (DATABASE*)NULL;
...
static void process_touch_command(conn *c, token_t *tokens, const size_t ntokens) {
    ...
    if (it) {
        ...
        process_item(it, enum_touch_item);
        item_remove(it);
    } else {
        pthread_mutex_lock(&c->thread->stats.mutex);
        c->thread->stats.touch_cmds++;
    ...
    while (!stop) {
        ...
        case conn_nread:
            ...
            if (c->rbytes > 0) {
                ...
                c->rcurr += tocopy;
                c->rbytes -= tocopy;
                process_item((item*)c->item, enum_save_item);
                //printf("item->refcount[%u]\n",((item*)c->item)->refcount);
                if (c->rlbytes == 0) {
                    break;
                }
            }
...
int main (int argc, char **argv) {
    ...
    while (-1 != (c = getopt(argc, argv,
        ...
        case 'T':
            ...
            pTmpPath = strdup(optarg);
            if( NULL == pTmpPath ) {
                 if (settings.verbose > 0) {
                        fprintf(stderr, "the database is NULL\n");      
                    }
            } else {
                if( strlen(pTmpPath) < DB_NAME_MAX_LEN ) {
                    memmove( settings.szDBName , pTmpPath , strlen(pTmpPath) );
                    settings.usePrs = true;
                    free( pTmpPath );
                    pTmpPath = NULL;
                } 
            }
